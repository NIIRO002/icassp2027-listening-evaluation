(() => {
  ...
  makeScale("naturalnessScale", "naturalnessCmos", [-3, -2, -1, 0, 1, 2, 3]);
  makeScale("similarityScale", "similarityCmos", [-3, -2, -1, 0, 1, 2, 3]);
  makeScale("confidenceScale", "confidence", [1, 2, 3, 4, 5]);
  document.querySelectorAll('input[name="abChoice"]').forEach((input) => input.addEventListener("change", updateNextState));
  ...
  function buildPresentationTrials(participantId) {
    ...
    return data.trials.map((trial, index) => {
      ...
      return { ...trial, audio_a: swapped ? trial.audio_right : trial.audio_left, audio_b: swapped ? trial.audio_left : trial.audio_right, presentation_code: swapped ? "R-L" : "L-R" };
    });
  }
  function resetTrialInputs() {
    document.querySelectorAll('#trialView input[type="radio"]').forEach((input) => { input.checked = false; });
    ended = {}; playCounts = {};
    ["endedReference", "endedA", "endedB"].forEach((id) => { byId(id).textContent = "끝까지 재생해 주세요"; byId(id).classList.remove("complete"); });
  ...
  function renderTrial() {
    ...
    resetTrialInputs();
    const needsReference = trial.trial_type === "cmos";
    byId("referenceRow").classList.toggle("hidden", !needsReference);
    byId("cmosPanel").classList.toggle("hidden", !needsReference);
    byId("abPanel").classList.toggle("hidden", needsReference);
    ...
    byId("choiceLegend").textContent = trial.choice_legend || "선택";
    setAudio(byId("audioA"), trial.audio_a);
    setAudio(byId("audioB"), trial.audio_b);
    if (needsReference) setAudio(byId("referenceAudio"), trial.reference_audio);
  ...
  function updateNextState() {
    if (!session) return;
    const trial = session.trials[trialIndex];
    const listened = ended.a && ended.b && (trial.trial_type !== "cmos" || ended.reference);
    const answered = trial.trial_type === "cmos" ? checked("naturalnessCmos") && checked("similarityCmos") : checked("abChoice") && checked("confidence");
    byId("nextButton").disabled = !(listened && answered);
  }
  function saveResponse() {
    const trial = session.trials[trialIndex];
    responses.push({
      study_id: data.study_id, participant_id: session.participantId, experience: session.experience, device_type: session.deviceType,
      participant_layout: session.layout, trial_index: trialIndex + 1, trial_id: trial.trial_id, block: trial.block, trial_type: trial.trial_type,
      presentation_code: trial.presentation_code, response_time_sec: ((performance.now() - trialStart) / 1000).toFixed(3), completed_at_utc: new Date().toISOString(),
      naturalness_ab_cmos: checked("naturalnessCmos"), similarity_ab_cmos: checked("similarityCmos"), response_choice: checked("abChoice"), confidence: checked("confidence"),
  ...
  byId("startButton").addEventListener("click", () => {
    ...
    if (!participantId || !byId("consent").checked) { byId("setupError").textContent = "참가자 ID와 연구 참여 동의를 확인해 주세요."; return; }
    const layout = hashParticipantId(participantId) % 2;
    session = { participantId, experience: byId("experience").value, deviceType: byId("deviceType").value, layout, trials: buildPresentationTrials(participantId) };
  ...
  byId("nextButton").addEventListener("click", () => {
    ...
    trialIndex += 1;
    if (trialIndex >= session.trials.length) { byId("progressText").textContent = "완료"; byId("progressBar").style.width = "100%"; showView("completeView"); return; }
  ...
  byId("downloadButton").addEventListener("click", () => {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `${data.study_id}_${session.participantId.replace(/[^a-zA-Z0-9_-]/g, "_")}.csv`; link.click(); URL.revokeObjectURL(url);
